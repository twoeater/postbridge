...
function clientMetadataProblem(value: unknown): string | undefined {
  ...
  if (!scopes.every((scope) => OAUTH_SCOPES.includes(scope as (typeof OAUTH_SCOPES)[number]))) {
    return "Only the blog:publish scope is supported";
  }
  return undefined;
}
...
<head>
  ...
  <title>${escapeHtml(config.displayName)} 승인</title>
  <style>
...
<body>
  <main>
    <h1>${escapeHtml(config.displayName)} 연결 승인</h1>
    <p><strong>${escapeHtml(clientName)}</strong>이 이 서버의 MCP 도구 사용 권한을 요청했습니다.</p>
    <p class="warning">승인하면 연결된 MCP 클라이언트가 ${escapeHtml(config.publicUrl)}에 새 글을 게시하거나 동일 slug의 글을 갱신할 수 있습니다.</p>
    ${invalidKey ? '<p class="error">인증키가 올바르지 않습니다.</p>' : ""}
    <form method="post" action="/authorize" autocomplete="off">
      ${fields}
      <label for="access_key">블로그 MCP 승인키</label>
...
export class BlogOAuthProvider implements OAuthServerProvider {
  ...
  private validateScopes(scopes: string[] | undefined): string[] {
    ...
    if (!requested.every((scope) => OAUTH_SCOPES.includes(scope as (typeof OAUTH_SCOPES)[number]))) {
      throw new InvalidScopeError("Only the blog:publish scope is supported");
    }
    return requested;
  }
  ...
  async authorize(
    ...
    // RFC 9207: bind the authorization response to this issuer to prevent mix-up attacks.
